...
stateDiagram-v2
    ...
    PENDING --> IDLE : byte < 0x80 (discard pending, emit RECEIVED for the short-form byte)
```
...
The host parser timeout (64 ms) is set once per frame, not per byte. Specifically, the deadline is set only when `deadline_ms == 0` (i.e., at the start of a new frame). If 64 ms elapses before the frame completes, the parser resets to IDLE and emits `ERROR_HOST`. This prevents a stale partial frame from corrupting subsequent decoding.

## Startup State Machine

The startup state enum tracks the firmware lifecycle from power-on through live operation and degraded mode. In the current host-buildable tree, only a simplified subset of this flow is implemented.
...
| 1 | CACHE_LOADED_STALE | Reserved/documented state; not currently exercised by the host-buildable runtime |
...
stateDiagram-v2
    [*] --> BOOT_INIT
    BOOT_INIT --> CACHE_LOADED_STALE : documented target flow
    CACHE_LOADED_STALE --> LIVE_WARMUP : documented target flow
...
The current runtime transitions to DEGRADED when:
