...
| Evaluation rule | The provider returns one mode/reason/revision verdict, but structural mode and acquisition health are independent. Fresh coherent identity plus an interpretable configuration establishes `INTERPRETED`; fresh coherent identity plus a known unsupported configuration establishes `GPIO_ONLY`; fresh coherent absence establishes `ABSENT`. A transient acquisition failure does not produce a new structural classification, so the previous coherent `INTERPRETED` mode remains unchanged. |
| Coherence / linearization rule | Relevant device-registry identity mutations advance a monotonic observation generation. The provider's final registry-generation comparison and semantic verdict commit execute in one registry read critical section, so a registry writer linearizes either before both operations or after both operations. A generation mismatch records `INCOHERENT_ACQUISITION` against retained state and does not commit a structural verdict from a detached registry snapshot. |
| Fallback / unknown behavior | The provider emits `GPIO_ONLY` only from fresh coherent `CONFIGURATION_NOT_INTERPRETABLE` evidence. Failed controller, configuration, solar, or cylinder acquisition retains the previous coherent structural mode and values without refreshing them. Before the first coherent classification, incomplete evidence leaves the structural verdict unpublished rather than inventing `GPIO_ONLY` or `ABSENT`. The full closed reason and precedence contract is [`../api/eebus-operator-admin.md#fm5-behavioral-verdict`](../api/eebus-operator-admin.md#fm5-behavioral-verdict). |
...
| Evaluation rule | A coherent `INTERPRETED` acquisition may create or update `solar`. A transient acquisition reason (`CONTROLLER_UNREACHABLE`, `CONFIGURATION_UNAVAILABLE`, `SOLAR_ACQUISITION_FAILED`, `CYLINDER_ACQUISITION_FAILED`, `EVIDENCE_STALE`, or `INCOHERENT_ACQUISITION`) retains the last coherent solar snapshot without updating it and leaves the previous coherent structural mode unchanged; the reason/revision marks the retained values non-current. Fresh coherent `ABSENT` or `GPIO_ONLY / CONFIGURATION_NOT_INTERPRETABLE` withdraws the family because the structural gate is no longer admissible. |
| Fallback / unknown behavior | No partial or synthetic solar snapshot is created from a failed acquisition. Transient failure may retain only an already coherent snapshot and must not present it as a new live acquisition. |
...
| Evaluation rule | A coherent `INTERPRETED` acquisition may create/update cylinder instances. A transient acquisition reason retains the last coherent instance set without updating it and leaves the previous coherent structural mode unchanged; the verdict marks retained values non-current. Fresh coherent `ABSENT` or `GPIO_ONLY / CONFIGURATION_NOT_INTERPRETABLE` withdraws the family because the structural gate is no longer admissible. |
...
| Constraint strength | `NON_AUTHORITATIVE` |
| Scope of validity | `GATEWAY_POLICY` |
| Evaluation rule | `hasLiveCylinderEvidence()` requires a decodable `temperatureC` to create or update an instance; config-only payloads do not create one. During an SD-13 transient acquisition reason, an already coherent instance may be retained unchanged without satisfying a new live-temperature read. |
