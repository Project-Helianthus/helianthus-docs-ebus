...
| Coherence / linearization rule | Relevant device-registry identity mutations advance a monotonic observation generation. The provider's final registry-generation comparison and semantic verdict commit execute in one registry read critical section, so a registry writer linearizes either before both operations or after both operations. A generation mismatch observed before that critical section returns `GPIO_ONLY / INCOHERENT_ACQUISITION`; it may not publish `INTERPRETED` from a detached registry snapshot. |
