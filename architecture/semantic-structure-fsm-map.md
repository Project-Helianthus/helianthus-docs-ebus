...
| `fm5SemanticMode` | System + radio + solar/cylinder evidence | FM5 semantic evaluation | Startup semantic FSM | Yes | Provider verdict becomes `GPIO_ONLY` with a closed reason while current or retained admissible identity evidence exists; it becomes `ABSENT` only when no admissible evidence remains |
| `solar` | FM5 + `GG=0x04` | Solar family gate | Startup semantic FSM | Yes | A transient `GPIO_ONLY` acquisition reason retains the last coherent snapshot without updating or zeroing it; `ABSENT` or `GPIO_ONLY / CONFIGURATION_NOT_INTERPRETABLE` withdraws it; with no retained snapshot the non-null object is empty |
| `cylinders` | FM5 + `GG=0x05` + per-instance temperature evidence | Cylinder family gate + individual cylinder gate | Startup semantic FSM | Yes | A transient `GPIO_ONLY` acquisition reason retains the last coherent instance set without updating or zeroing it; `ABSENT` or `GPIO_ONLY / CONFIGURATION_NOT_INTERPRETABLE` withdraws it; with no retained instances the non-null list is empty |
