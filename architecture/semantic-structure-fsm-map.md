...
| `fm5SemanticMode` | System + radio + configuration evidence | FM5 structural evaluation | Startup semantic FSM | Yes | A transient acquisition failure retains the previous coherent mode; `GPIO_ONLY` requires fresh coherent `CONFIGURATION_NOT_INTERPRETABLE` evidence and `ABSENT` requires fresh coherent absence evidence |
| `solar` | FM5 + `GG=0x04` | Solar family gate | Startup semantic FSM | Yes | A transient acquisition reason retains the last coherent snapshot without updating or zeroing it; fresh coherent `ABSENT` or `GPIO_ONLY / CONFIGURATION_NOT_INTERPRETABLE` withdraws it; with no retained snapshot the non-null object is empty |
| `cylinders` | FM5 + `GG=0x05` + per-instance temperature evidence | Cylinder family gate + individual cylinder gate | Startup semantic FSM | Yes | A transient acquisition reason retains the last coherent instance set without updating or zeroing it; fresh coherent `ABSENT` or `GPIO_ONLY / CONFIGURATION_NOT_INTERPRETABLE` withdraws it; with no retained instances the non-null list is empty |
