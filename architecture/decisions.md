...
**Decision:** Encode ENH as two bytes where the first carries the command and the high two data bits, and the second carries the remaining six data bits. The parser emits either raw data bytes or command frames.

**Consequences:** Transport decodes ENH frames and only forwards data bytes (from `ENHResReceived`) to the Bus.
