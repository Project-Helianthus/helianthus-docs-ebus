...
| Supporting statement | In `4.2.1 VR 92 Remote Control Unit`, the VRC 720 training says "Each VR 92 is assigned to a specific zone". In `Zone Assignment with Remote Controls`, it also documents that remote controls determine zone assignment during commissioning. This supports the existence of explicit zone-to-room-sensor attachment semantics, although the numeric B524 mapping values are still proven primarily by register evidence. |
...
| Supporting statement | In `Zonas 1 a 9`, the multiMATIC training exposes `Asignacion de zona` for each zone. In `Zone Assignment with Remote Controls`, the VRC 720 training warns that if a circuit is used as something other than a heating circuit "the zone assignment must be changed manually". This supports the existence of explicit zone-to-circuit assignment, but does not prove that the gateway fallback-to-zone-instance branch is universally correct. |
...
| Source document title | `Vaillant sensoCOMFORT (VRC 720) -- Training Document`; `Vaillant multiMATIC VRC 700/4f, VRC 700/5 - Control por compensacion climatica` |
| Supporting statement | Across `4.2.1 VR 92 Remote Control Unit`, `4.3.1 VR 71 Main Connection Center`, `Zone Assignment with Remote Controls`, `Installer Level -- Solar Circuit`, and the multiMATIC sections `Distribucion de zonas y circuitos de calefaccion`, `Zonas 1 a 9`, and `Circuito solar 1`, the regulator documents consistently present structure as explicit relationships: zones have room controls and circuit assignments, circuits have types and actuator state, VR 71 owns relay-backed functions, and solar/cylinder screens appear only for certain configurations. This matches the gateway design choice to publish explicit structural fields instead of hidden thresholds. |
