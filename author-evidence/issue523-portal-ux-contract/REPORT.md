...
The exact `3644a8c12a39217b383fa4831738fe9d8476dd33` GitHub review found two
P2 defects. Functional/report commit
`a24e0dc780b579d9d7a470c3be3aeb4b443c5847`, tree
`e92918170d4b21926869a822501fab915f8967e5`, recognizes `/graphql` as a route
independently of the HTTP method so an added `GET /graphql` contradicts the
frozen `POST` contract and rejects. It also corrects the report's `Refreshing`
summary: triggering READ success returns `Active`; triggering current-owner
DISABLE success reaches `Idle` only after a valid disable ACK; later bus-facing
operations remain busy. Complete configured CI passes with 270 Portal checker
tests and 121 canonical B503 milestone tests. Its log is
`wave12/ci/docs524-a24e0dc-graphql-refresh-ci.log`, SHA-256
`5600e46cd79a30c28143dbb0d898090a90d65cf6a5f6930be83c2bff7639a925`. The
...
The fresh independent review of exact
`5f2e0c1560f25bd054d93b8dcc590cb8132f359c`, tree
`53285875fdfc4ad924cd182f1d107baac484cc1f`, verified all earlier P2
corrections and found one scope gap in the same automatic-restart-write rule:
the contradiction rejected in §7.5 still passed when appended in normative §6
or §8. Its report has SHA-256
`74cd622bfbcd7ba5f9fad7320fd4e7d3aeef3de9d5a0d1d8eada9f617e3151b0`.
Commit `2fec42e431e88ebaf8d60e45f597a7f89ddc4a94`, tree
`4c2b2154a75fbfbaf9f7c4e350d72091d9ecb6bc`, applies that one bounded
contradiction rule to the complete declared §§6–8 session-contract scope.
Mutations in §6, §7.5, and §8 reject, while the separately
operator-authorized target-specific recovery remains accepted. The canonical
...
The fresh independent review of exact
`a9a432164b4f3c0cbb5f55d3a320bf6de7783592`, tree
`4fd32f04d8e3d263cd7da0ecefe32f1da367f0a5`, returned
`NO_BLOCKING_FINDINGS`; its report is
`wave12/review/docs524-a9a4321-final-independent/REPORT.md`, SHA-256
`9aaea2abdf402732c30bfe01c6b68b7f1c51fafcf5fab151c6f8e56f3cd9de3e`.
A post-review live capture then received a new GitHub P2: the exact §12.5
capability table was still parsed from raw Markdown and could be moved alone
into a fence. Functional commit
`ef5476f528d8fea9e5553465effd7e8126bf2939`, tree
`89e3f7e81baa234f7972a738f0a47a56ec657cbb`, parses every canonical declared
prose/table artefact from non-code source: the §12.5 capability table, the §14
milestone table, and the exact §9 install-write contract. Focused mutations
fence each artefact independently and reject; the current visible forms pass.
The Portal checker passes 285 tests, the canonical checker passes 146 tests,
...
The subsequent GitHub review of exact
`bc4bb30efa11082e4fd75902559af3e2b981d7ba` found three P2 defects. The
refreshed READ row retained `ACTIVE` even when that dispatched READ encountered
a new transport disconnect, image alternative text could still supply a
declared literal anchor, and the command table retained the prior 146-test
count. Functional commit `950519b3bf915c38fde23fc1594958a61e1fe951`,
tree `9e1aa3b83b722fdaed31e2e9c71d1334e9e126cd`, scopes the `REFRESHING → ACTIVE`
READ outcome to a dispatch that completes without transport disconnect. A
disconnect follows the global disconnect transition, releases the owner into
internal `DISABLED`, and retains applicable cleanup. Declared literal anchors
now exclude image tokens as well as link metadata and code; the exact
image-alternative mutation rejects. The command table above records the actual
