...
takes the non-shadow decision path (check `entry.running` first for coalesced
in-flight read, then `entry.lastOK` scheduler-cache hit when valid for
max-age+generation, otherwise active fetch, breaker suppression failure, or
active read error).
